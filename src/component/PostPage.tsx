...
const PostPage = () => {
  ...
  const { id } = useParams();
  const postPageFilter = posts.filter((post) => post._id.toString() === id);
      ...
      {postPageFilter.map((postPage:any) => {
        return (
          ...
          >
            ...
            <small>{postPage.created.slice(0,10)} <span>{postPage.created.slice(11,20)}</span></small> <br />
            <small>{postPage.body}</small> <br />
            <div className="flex gap-5">
              <button
                className="mt-3 mb-3 "
                onClick={() => navigate(`/edit/${postPage._id}`)}
              >
              ...
              <button
                ...
                type="button"
                onClick={() => handleDelete(postPage._id)}
              >
